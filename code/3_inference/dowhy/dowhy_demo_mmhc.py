...
def StandardOutput(am, method, k, path, data_type="RA", matrix_type="am"):
    os.chdir(path)
    print(os.getcwd())
    if (not os.path.exists(method+"_mmhc1")):
        os.makedirs(method+"_mmhc1")
    am = pd.DataFrame(am)
    am.columns = ["sp" + str(i) for i in range(am.shape[0])]
    am.index = ["sp" + str(i) for i in range(am.shape[0])]
    if (matrix_type == "am"):
        am.to_csv(method + "_mmhc1\\" + data_type + "_adjacent_matrix_" + str(k) + ".csv")
    elif (matrix_type == "p"):
        am.to_csv(method + "_mmhc1\\" + data_type + "_p_" + str(k) + ".csv")
...
def func(par):
    ...
    ##mmhc
    #am_aa = pd.read_csv(
    #    "C:\\Users\\Administrator\\Desktop\\causal_compare\\data\\" + par["i"] + "_" + par["j"] + "\\DoWhy_mmhc\\AA_adjacent_matrix_" + str(
    #        k) + ".csv", index_col=0)
    p_aa = pd.read_csv(
        "C:\\Users\\Administrator\\Desktop\\causal_compare\\data\\" + par["i"] + "_" + par["j"] + "\\DoWhy_mmhc\\AA_p_" + str(
            k) + ".csv", index_col=0)
    ...
    for s in range(p_aa.shape[0]):
        for t in range(p_aa.shape[1]):
            if(p_aa.ix[s, t] <= 0.05):
                am_aa.ix[s, t] = 1
    am_ra = pd.read_csv(
        "C:\\Users\\Administrator\\Desktop\\causal_compare\\data\\" + par["i"] + "_" + par["j"] + "\\DoWhy_mmhc\\RA_adjacent_matrix_" + str(
            k) + ".csv", index_col=0)
    p_ra = pd.read_csv(
        "C:\\Users\\Administrator\\Desktop\\causal_compare\\data\\" + par["i"] + "_" + par["j"] + "\\DoWhy_mmhc\\RA_p_" + str(
            k) + ".csv", index_col=0)
    ...
    ##mmhc
    am_aa = pd.read_csv("C:\\Users\\Administrator\\Desktop\\causal_compare\\beem_e\\" + par["i"] + "_" + par["j"] + "\\mmhc\\AA_adjacent_matrix_" + str(k) + ".csv", index_col=0)
    label = ["sp" + str(i) for i in range(len(am_aa))]
    am_aa = pd.DataFrame(am_aa, index=label, columns=label)

    am_ra = pd.read_csv("C:\\Users\\Administrator\\Desktop\\causal_compare\\beem_e\\" + par["i"] + "_" + par["j"] + "\\mmhc\\RA_adjacent_matrix_" + str(k) + ".csv", index_col=0)
    am_ra = pd.DataFrame(am_ra, index=label, columns=label)
