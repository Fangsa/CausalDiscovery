...
def StandardOutput(am, method, k, path, data_type="RA", matrix_type="am"):
    os.chdir(path)
    print(os.getcwd())
    if (not os.path.exists(method+"_100")):
        os.makedirs(method+"_100")
    am = pd.DataFrame(am)
    am.columns = ["sp" + str(i) for i in range(am.shape[0])]
    am.index = ["sp" + str(i) for i in range(am.shape[0])]
    if (matrix_type == "am"):
        am.to_csv(method + "_100\\" + data_type + "_adjacent_matrix_" + str(k) + ".csv")
    elif (matrix_type == "p"):
        am.to_csv(method + "_100\\" + data_type + "_p_" + str(k) + ".csv")
...
def func(par):
    ...
    # DoWhy
    am_aa, p_aa = main(aa, am)
    print(am_aa)
    print(p_aa)
    am_ra, p_ra = main(ra, am)
    StandardOutput(am_aa, method="DoWhy_true", k=k, data_type="AA", matrix_type="am", path=path)
    StandardOutput(am_ra, method="DoWhy_true", k=k, data_type="RA", matrix_type="am", path=path)
    StandardOutput(p_aa, method="DoWhy_true", k=k, data_type="AA", matrix_type="p", path=path)
    StandardOutput(p_ra, method="DoWhy_true", k=k, data_type="RA", matrix_type="p", path=path)
    print("running time is "+str(time.time()-start_time))

if __name__ == "__main__":
    # dat = pd.read_table(r"C:\Users\Administrator\Desktop\causal_compare\data\gLV_CS\AA_1.txt", index_col=0).T
    # dat = CreatData(dat, am, treatment="sp0", outcome="sp1")
    # causal_estimate = CalDoWhy(dat)
    # pvalue = CalPvalue(dat, causal_estimate.value)
    pool = mp.Pool(processes=30)
    for k in range(1, 11):
        # Creat pesudo adjacent matrix, half of the relationship is accurate by default.
        ...
        for i in ["gLV", "hubbell", "soi"]:#
            for j in ["CS", "TS"]:#
                # os.chdir(r"C:\\Users\\Administrator\\Desktop\\causal_compare\\data\\" + i + "_" + j)
                path = "C:\\Users\\Administrator\\Desktop\\causal_compare\\data\\" + i + "_" + j
                # dat = pd.read_table(r"AA_1.txt", index_col=0).T
                par = {
                    "k": k,
                    "am": am,
                    "path": path
