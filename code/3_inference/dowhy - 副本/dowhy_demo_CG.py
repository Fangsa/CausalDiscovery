...
def StandardOutput(am, method, k, path, data_type="RA", matrix_type="am"):
    os.chdir(path)
    print(os.getcwd())
    if (not os.path.exists(method+"_cg1")):
        os.makedirs(method+"_cg1")
    am = pd.DataFrame(am)
    am.columns = ["sp" + str(i) for i in range(am.shape[0])]
    am.index = ["sp" + str(i) for i in range(am.shape[0])]
    if (matrix_type == "am"):
        am.to_csv(method + "_cg1\\" + data_type + "_adjacent_matrix_" + str(k) + ".csv")
    elif (matrix_type == "p"):
        am.to_csv(method + "_cg1\\" + data_type + "_p_" + str(k) + ".csv")
...
def func(par):
    ...
    p_aa = pd.read_csv(
        "C:\\Users\\Administrator\\Desktop\\causal_compare\\data\\" + par["i"] + "_" + par["j"] + "\\DoWhy_cg\\AA_p_" + str(
            k) + ".csv", index_col=0)
    ...
    p_ra = pd.read_csv(
        "C:\\Users\\Administrator\\Desktop\\causal_compare\\data\\" + par["i"] + "_" + par["j"] + "\\DoWhy_cg\\RA_p_" + str(
            k) + ".csv", index_col=0)
