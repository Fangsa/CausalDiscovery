...
def func(par):
    ...
    ##mmhc
    am_aa = pd.read_csv("C:\\Users\\Administrator\\Desktop\\causal_compare\\data\\" + par["i"] + "_" + par["j"] + "\\mmhc\\AA_adjacent_matrix_" + str(k) + ".csv", index_col=0)
    label = ["sp" + str(i) for i in range(len(am_aa))]
    am_aa = pd.DataFrame(am_aa, index=label, columns=label)

    am_ra = pd.read_csv("C:\\Users\\Administrator\\Desktop\\causal_compare\\data\\" + par["i"] + "_" + par["j"] + "\\mmhc\\RA_adjacent_matrix_" + str(k) + ".csv", index_col=0)
